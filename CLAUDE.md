...
> **الاسم موضع (Mawdi).** كل نص وواجهة عامة تقول «موضع»/Mawdi (النصوص، كود الخصم `MAWDI50`، مفتاح اللغة `mawdi_lang`).
> **استثناء موثّق:** بادئة مرجع الإعلان `SK-` (لا `MW-`) — قرار بتاريخ ٢٠٢٦-٠٩-١٤ بإرجاعها لـ«سكن» عمداً رغم إن باقي الواجهة العامة «موضع». المصدر الوحيد: `set_listing_ref()` (تريغر على `listings`) + `listing_ref_seq` (أُعيد ضبطه لـ١٠٠١ وقت التبديل — migration `20260914145004_listing_ref_prefix_sk`).
...
16. **الأدوار من القاعدة (`staff`) مش من الواجهة.** الفاعل يُشتق حصراً من `auth.uid()` عبر `actor_name()`/`is_staff()`/`is_admin()`. بقاء معطى `p_actor` بتوقيع `admin_set_setting`/`admin_listing_status`/`admin_report_status` استثناء موثّق: قيمة احتياطية لا تُستخدم إلا حين يكون `auth.uid()` فارغاً (`service_role`/SQL مباشر)، ولا تتجاوز حارس `is_staff()`/`is_admin()` أبداً (تحقّق عملياً بتدقيق ٢٠٢٦-٠٩-٠٥: مستخدم `authenticated` عادي مرّر `p_actor` بقيمة `'admin'` وانرفض بـ"غير مصرّح" رغم ذلك). **ممنوع إضافته لأي دالة جديدة**، وممنوع جعله مصدر الفاعل حين يكون `auth.uid()` موجوداً.
