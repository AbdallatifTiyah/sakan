...
> **استثناء موثّق:** بادئة مرجع الإعلان `SK-` (لا `MW-`) — قرار بتاريخ ٢٠٢٦-٠٩-١٤ بإرجاعها لـ«سكن» عمداً رغم إن باقي الواجهة العامة «سكنّا». `listing_ref_seq` (أُعيد ضبطه لـ١٠٠١ وقت التبديل — migration `20260914145004_listing_ref_prefix_sk`).
> **الرقم نفسه (بعد البادئة) بيتولّد بآخر سطر بدالة `submit_listing()` عمداً** (`update listings set ref = 'SK-' || nextval('listing_ref_seq') ... returning ref` — آخر عملية كتابة بالدالة قبل `return` مباشرة)، **مش بالتريغر** — قرار بتاريخ ٢٠٢٦-٠٩-٢٣ (migration `20260923185807_submit_listing_gapless_ref`) لضمان تسلسل بدون فجوات: أي فشل بمنتصف الدالة (نوع سكن غلط، خرق قيد، إلخ) بيصير **قبل** استدعاء `nextval()`، فما يستهلك رقماً بدون ما ينتج إعلان فعلي. `submit_listing()` بيسجّل الصف مبدئياً بقيمة `ref` مؤقتة فريدة (`'PENDING-' || gen_random_uuid()`) عشان يتجاوز التريغر (يشتغل بس لو `new.ref is null`). التريغر `set_listing_ref()` (`trg_listing_ref`, `BEFORE INSERT` على `listings`) **لسا موجود كشبكة أمان** لأي إدراج مباشر لـ`listings` من برّا `submit_listing()` (نادر/يدوي) — بس المسار العادي (كل إعلان عام) ما بيعدّي منه فعلياً.
...
- **`listings.rented_via_platform`** (boolean، nullable — migration `20260924081609`): لما مندوب/طاقم يعلّم إعلان «مؤجّر» من اللوحة (`admin_listing_status(..., p_rented_via_platform)`)، بينسأل سؤال نعم/لا: تأجّر عن طريق المنصة فعلاً (باحث تواصل من عندنا وأدّى للتأجير) ولا خارجياً (المالك أجّر لشخص وصله بطريقة تانية، بس حابب يوقف الإعلان)؟ **`median_days_to_rent`** (`v_kpi_quality`) بيستثني الصفوف المعلَّمة `false` صراحة (`rented_via_platform IS NOT FALSE` — يعني `true` أو `null` القديمة بتحسب زي السابق، بس `false` الصريحة بتُستثنى) — منع تلوّث مؤشر جودة حقيقي بحالات تأجير ما لها علاقة بالمنصة. **لا علاقة بالرسم إطلاقاً** — الرسم (`owner_fees`) بينخلق فقط لما **طلب تواصل محدد** (`contact_requests`) يُعلَّم «مؤجّر» عبر `admin_contact_status`/`on_contact_rented`، مش لما حالة الإعلان نفسها تتغيّر. تعليم إعلان «مؤجّر» بدون طلب تواصل مرتبط = صفر أثر على الفوترة بغض النظر عن قيمة هالعمود.
...
| بادئة مرجع الإعلان (السلسلة الحرفية `SK-` نفسها) طلعت غلط أو محتاج تتغيّر | البادئة الحرفية `'SK-'` مكتوبة حرفياً بالسطر الأخير من `submit_listing()` (وباحتياط بـ`set_listing_ref()` للإدراج المباشر). أي تغيير للبادئة = تعديل الاثنين بنفس الـmigration + `alter sequence listing_ref_seq restart with <n>`. |
| رقم `SK-` قفز أكتر من واحد (فجوة بالتسلسل، زي `SK-1014` وبعده مباشرة `SK-1018`) | كان طبيعياً قبل ٢٠٢٦-٠٩-٢٣: التريغر (`BEFORE INSERT`) كان يستدعي `nextval()` **وقت الإدراج**، فأي محاولة إرسال تفشل بمنتصف `submit_listing()` (كود نوع سكن غلط، خرق قيد) كانت تستهلك رقماً دون ما تنتج إعلان — والـsequence بـPostgres مش transactional، الرقم ما بيرجع حتى لو صار rollback كامل. بعد migration `20260923185807_submit_listing_gapless_ref`، الرقم بيتولّد بآخر سطر بالدالة (بعد ضمان النجاح)، فهاي الفجوات ما بتصير من جديد لأي إعلان يعدّي من `submit_listing()`. فجوة قديمة لو انلاحظت = مجرد أثر تاريخي، مش إعلان مفقود ولا خلل بالبيانات — ما تحتاج تصحيح. |
